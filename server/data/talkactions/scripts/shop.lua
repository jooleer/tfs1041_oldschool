...
function onSay(cid, words, param)
	local storage = 30014 -- Make sure to select non-used storage. This is used to prevent SQL load attacks.
	local cooldown = 30 -- in seconds.
	...
	if player:getStorageValue(storage) <= os.time() then
		...
		if orderQuery ~= false then
			...
			if q_type == 1 then
				...
				if player:getFreeCapacity() >= ItemType(q_itemid):getWeight(q_count) then
					...
					player:sendTextMessage(MESSAGE_INFO_DESCR, "You have successfully redeemed " .. q_count .. " " .. ItemType(q_itemid):getName() .. ".")
				else
					player:sendTextMessage(MESSAGE_STATUS_WARNING, "You have no capacity.")
				end
			end
		...
		player:sendTextMessage(MESSAGE_STATUS_WARNING, "You have to wait " .. player:getStorageValue(storage) - os.time() .. " seconds to check your orders again.")
	end
	return false
end