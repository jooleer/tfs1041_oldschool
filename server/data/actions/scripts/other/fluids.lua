...
-- start of In Service for the King
	if target.uid == 6004 and item.type == 0 and player:getStorageValue(INSERVICESTORAGE) == 6 then
		item:transform(item:getId(), 3)
		player:setStorageValue(INSERVICESTORAGE, 7)
		player:sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, 'The container is full with poisonous goblin beer.')		
		return true
	end
	if target.uid == 6005 and item.type == 3 and player:getStorageValue(INSERVICESTORAGE) == 10 then
		item:transform(item:getId(), 0)
		player:setStorageValue(INSERVICESTORAGE, 11)
		player:sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, 'The cauldrom is full with poisonous goblin beer.')
		return true
	end
-- end of In Service for the King	
	...
	if target:isCreature() then
		...
		elseif target == player then
			...
			elseif item.type == 7 then
				player:addMana(math.random(40, 75))
