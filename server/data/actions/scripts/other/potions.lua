...
-- CELESTIA: lowered it to 0,5sec as in real 7.4 it had no exhaust at all.

function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if (target == nil) or not (target == player) then
		return false
	end
	...
	elseif itemId == manaPot then
		if not doTargetCombatMana(0, target, 40, 75, CONST_ME_MAGIC_BLUE) then
			return false
		end
