...
std::string Player::getDescription(int32_t lookDistance) const
{
	...
	/* 
	// Removing Party and Guild information from LOOK message.
	if (party) {
		if (lookDistance == -1) {
			s << " Your party has ";
	...
	*/
	if (guild) {
		...
		if (rank) {
			...
			// Adding period in the end of the message (part of the Removing Party and Guild information from LOOK message change)
			s << ".";
			/*
			// Removing Party and Guild information from LOOK message.
...
void Player::addExperience(Creature* source, uint64_t exp, bool sendText/* = false*/)
{
	...
	if (prevLevel != level) {
		/*
		// Not allowing players to restore full hp and mana on level up
		health = healthMax;
		mana = manaMax; */
...
void Player::doAttacking(uint32_t)
{
	...
	if (hasCondition(CONDITION_PACIFIED) && !attackedCreature->getPlayer()) {
		return;
	}
